...
- Unified goal shaping (XY) with reach multiplier.
- Orientation alignment rewards gated by proximity to goal.
...
def _get_angle_diff_to_goal(env: ManagerBasedRLEnv) -> torch.Tensor:
    """Shortest rotation angle (radians) between object and goal quaternions."""
...
def ms_fine_alignment_reward(env: ManagerBasedRLEnv) -> torch.Tensor:
    """Fine rotation tuning when already very close (dist < 2cm).

    R = (1 - tanh(20 * angle_diff)) * (dist < 0.02)
    Very steep gradient to reward the last fraction of a radian.
    """
    dist = _get_dist_to_goal_2d(env)
    angle_diff = _get_angle_diff_to_goal(env)
    fine_align = 1.0 - torch.tanh(20.0 * angle_diff)
    close_mask = (dist < 0.02).float()
