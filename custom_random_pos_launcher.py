...
# Ensure the new environment package is imported so that its gym envs are registered
import push_random_pos_cube  # noqa: F401

# Add rsl_rl training scripts to path
sys.path.append(os.path.join(isaaclab_root, "scripts/reinforcement_learning/rsl_rl"))

# Configure training arguments for the new task
sys.argv = [
    "train.py",
